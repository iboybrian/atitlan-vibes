...
**Picked town vs. detected town are different columns and must stay that way.** `current_town_id` is a stated preference and drives everything the user *sees*. `users.detected_town_id` is where geolocation last placed them and only affects *who push targets* — fresh detection (<24h) wins, otherwise targeting falls back to the pick. Never let one overwrite the other: hijacking the picker would yank a user who deliberately chose San Marcos back to wherever their phone is.
