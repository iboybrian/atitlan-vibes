...
import { Clock, Plus, ChevronDown, Ship, MessageCircle, Search, X } from 'lucide-react'
...
export default function TownDetail() {
    ...
    return (
        <div className="px-4 py-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {filteredEvents.map(event => (
                        <Link to={`/event/${event.id}`} key={event.id} className="block group">
                            <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer h-full">
            ...
            {/* Floating Chat Button (logged-in users only) */}
            {user && (
                <Link
                    to={`/town/${id}/chat`}
                    className="fixed bottom-6 right-6 w-14 h-14 bg-turquoise text-white rounded-full shadow-lg hover:shadow-xl hover:scale-110 transition-all flex items-center justify-center z-40"
                    title="Town Chat"
                >
